...
class ActionsDolitypology extends CommonHookActions
{
	...
	public function printCommonFooter($parameters, &$object, &$action)
	{
		...
		if (strpos($parameters['context'], 'productcard') !== false) {
			...
			$out = $extrafields->showSeparator('dolitypo_typo', $object);

			$extratest = new TypologyExtrafieldLink($db);
			$a = $extratest->fetchAll('', '', 0, 0, ['customsql' => 't.fk_typology = ' . $object->array_options['options_dolitypo_typo']]);
			//$out = '';
			foreach ($a as $row) {
				$b = fetch_extrafields_id($row->fk_extrafield, 'product');
				$out .= '<tr><td>' . $b[0]->label . '</td>';
				$extrafields = new ExtraFields($db);
				$extrafields->fetch_name_optionals_label('product');
				$out .= '<td>' . $extrafields->showOutputField($b[0]->name, $object->array_options['options_' . $b[0]->name], '', $object->table_element, null, $object) . '</td></tr>';
			}

			?>
			<script>
				jQuery('#product_extras_dolitypo_typo_<?php echo $object->id ?>').closest('tr').after(<?php echo json_encode($out) ?>);
			</script>
			<?php
		}
